 ...
 * Distributor class.
 *
 * The `Distributor` class orchestrates the distribution of posts from the
 * originating site to connected blogs and remote networks. It schedules
 * asynchronous tasks using the Action Scheduler library, listens for
 * cron actions to process queued items, and provides methods to
 * distribute single posts or batches of posts. By centralising the
 * scheduling and execution logic in this class, the plugin ensures that
 * distribution happens reliably even on large multisite installations.
 * Extend this class to add new distribution workflows or to tweak the
...
class Distributor {
	...
	public function __construct() {
		...
		add_action( 'cron_action_start_queue_items', array( $this, 'start_queue_items' ), 10, 2 );

		add_action( 'contentsync_distributor_distribute_item', array( $this, 'distribute_item' ), 10, 1 );

		add_action( 'contentsync_before_import_global_posts', array( $this, 'before_import_global_posts' ), 10, 2 );
		add_action( 'contentsync_after_import_global_posts', array( $this, 'after_import_global_posts' ), 10, 2 );
	}
	 ...
	 */
	public static function distribute_single_post( $root_post, $destination_ids_or_arrays = array(), $export_args = array() ) {
	 ...
	 * @return WP_Error|true  WP_Error on failure, true on success.
	 */
	public static function distribute_posts( $post_ids_or_objects, $destination_ids_or_arrays = array(), $export_args = array() ) {
		...
		if ( count( $post_ids_or_objects ) > $chunk_size ) {
			...
			foreach ( $chunks as $chunk ) {
				$result = self::distribute_posts( $chunk, $destination_ids_or_arrays, $export_args );
	 ...
	 * @return WP_Error|true  WP_Error on failure, true on success.
	 */
	public static function distribute_posts_per_blog( $posts_keyed_by_blog, $destination_ids_or_arrays = array(), $export_args = array() ) {
	 ...
	 */
	public static function prepare_posts_for_distribution( $post_ids_or_objects, $export_args = array(), $root_post_id = 0 ) {
	...
	public static function schedule_post_distribution( $prepared_posts, $destinations ) {
		...
		foreach ( $destinations as $destination ) {
			...
			$distribution_item = self::schedule_distribution_item( $distribution_item_properties );
	...
	public static function schedule_distribution_item( $distribution_item_properties ) {
		...
		$result = as_schedule_single_action(
			time(),
			'contentsync_distributor_distribute_item',
	...
	public static function distribute_to_blog( $item ) {
		...
		if ( isset( $item->destination->import_action ) && $item->destination->import_action == 'delete' ) {
			...
			$result = self::import_posts_to_blog( $item );
		}
