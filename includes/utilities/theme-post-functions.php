<?php
/**
 * Theme post helper functions.
 *
 * This file contains helper functions for working with theme posts such as templates, template parts, global styles, etc.
 */
 ...
 */
function get_theme_posts( $args ) {

	$args = wp_parse_args(
		$args,
		array(
			'post_type'      => get_theme_post_types(),
			'post_status'    => 'publish',
			'posts_per_page' => -1,
		)
	);
 ...
 */
function set_wp_template_theme( $post, $switch_references_in_content = false ) {
 ...
 * @return bool|WP_Error True on success, WP_Error on failure.
 */
function set_wp_global_styles_theme( $post ) {
