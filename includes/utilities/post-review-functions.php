 ...
 * `Synced_Post_Review` objects by ID, by post and blog combination, or to
 ...
 * object construction performed by the `Synced_Post_Review` class. Use
...
use Contentsync\Synced_Post_Review;
use Contentsync\Synced_Post_Review_Message;
...
/**
 * Get a Synced_Post_Review object by ID
 *
 * @param int $post_review_id
 *
 * @return Synced_Post_Review|false
 */
function get_synced_post_review_by_id( $post_review_id ) {
	return Synced_Post_Review::get_instance( $post_review_id );
}

/**
 * Get a Synced_Post_Review object by post ID and blog ID
 *
 * @param int $post_id
 *
 * @return Synced_Post_Review|false
...
/**
 * Get all Synced_Post_Review objects by post ID and blog ID
 *
 * @param int $post_id
 *
 * @return Synced_Post_Review[]
 */
function get_contentsync_all_post_reviews_by_post( $post_id, $blog_id, $state = null ) {
	...
	foreach ( $post_reviews as $post_review ) {
		$post_review_objects[] = new Synced_Post_Review( $post_review );
	}
...
/**
 * Get all Synced_Post_Reviews depending on the state
 *
...
/**
 * Insert a new Synced_Post_Review
 *
...
/**
 * Set the state of a Synced_Post_Review
 *
 * @param int    $post_review_id
 * @param string $state
 *
...
/**
 * Add a message to a Synced_Post_Review
 *
 * @param int    $post_review_id
...
function get_messages_by_synced_post_review_id( $post_review_id ) {
	...
	// loop through the messages and create Synced_Post_Review_Message objects
	$messages = array_map(
		function ( $message ) use ( $post_review_id ) {
			return new Synced_Post_Review_Message( $post_review_id, $message );
		},
		$messages
	);
...
/**
 * Get the latest message of a Synced_Post_Review
 *
 * @param int $post_review_id
 *
 * @return Synced_Post_Review_Message|false
 */
function get_latest_message_by_synced_post_review_id( $post_review_id ) {
	...
	if ( ! $latest_message instanceof Synced_Post_Review_Message ) {
		$latest_message = new Synced_Post_Review_Message( $post_review_id, $latest_message );
	}
 ...
 */
function replace_posts_with_previous_version_before_distribution( $preparred_posts, $post_ids, $export_args ) {

	\Contentsync\Distribution\Logger::add( 'replace_posts_with_previous_version_before_distribution' );
	// \Contentsync\Distribution\Logger::add( 'preparred_posts', $preparred_posts );
	// \Contentsync\Distribution\Logger::add( 'post_ids', $post_ids );
	// \Contentsync\Distribution\Logger::add( 'export_args', $export_args );

	foreach ( $preparred_posts as $key => $post ) {

		$post_review = get_synced_post_review_by_post( $post->ID, get_current_blog_id(), array( 'new', 'in_review' ) );
		// \Contentsync\Distribution\Logger::add( 'post_review', $post_review );
		...
		if ( $post_review && $post_review->previous_post ) {
			...
			if ( $post_review->previous_post->post_status === 'auto-draft' ) {
				unset( $preparred_posts[ $key ] );
				continue;
			}

			$preparred_posts[ $key ] = $post_review->previous_post;
		}
	}
