...
class Review_Mail {
	...
	public static function send_review_mail( $review_id, $status, $recipient ) {
		...
		if ( $recipient === 'reviewers' ) {
			if ( $status === 'new' ) {
				...
				$content = Mails\get_mail_content_for_reviews_reviewer_new( $post_review, $post );
			} elseif ( $status === 'in_review' ) {
				require_once __DIR__ . '/Mails/review-reviewer-updated.php';
				$content = Mails\get_mail_content_for_reviews_reviewer_updated( $post_review, $post );
			}
		} elseif ( $recipient === 'editor' ) {
			if ( $status === 'approved' ) {
				require_once __DIR__ . '/Mails/review-editor-approved.php';
				$content = Mails\get_mail_content_for_reviews_editor_approved( $post_review, $post );
			} elseif ( $status === 'denied' ) {
				require_once __DIR__ . '/Mails/review-editor-denied.php';
				$content = Mails\get_mail_content_for_reviews_editor_denied( $post_review, $post );
			} elseif ( $status === 'reverted' ) {
				require_once __DIR__ . '/Mails/review-editor-reverted.php';
				$content = Mails\get_mail_content_for_reviews_editor_reverted( $post_review, $post );
			}
		}
	...
	public static function send_mail( $to, $subject, $message, $headers = '', $attachments = array() ) {
		...
		// set filter
		add_filter( 'wp_mail_content_type', array( self::class, 'set_html_mail_content_type' ) );
		add_filter( 'wp_mail_charset', array( self::class, 'get_utf8_charset' ) );
		add_action( 'wp_mail_failed', array( self::class, 'on_mail_error' ) );
		...
		remove_filter( 'wp_mail_content_type', array( self::class, 'set_html_mail_content_type' ) );
		remove_filter( 'wp_mail_charset', array( self::class, 'get_utf8_charset' ) );
		remove_action( 'wp_mail_failed', array( self::class, 'on_mail_error' ) );
	 ...
	 */
	public static function get_utf8_charset() {
		return 'UTF-8';
	}
	 ...
	 * @param string      $body   The main email body content.
	 * @param string|null $footer The footer content (optional).
	 *
	...
	public static function create_mail_template( $title, $body, $footer = null ) {
		...
		ob_start();
		require __DIR__ . '/Mails/html-template.php';
		return ob_get_clean();
	}
