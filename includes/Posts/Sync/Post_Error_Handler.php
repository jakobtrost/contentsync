...
/**
 * Static helper class for post error handling and repair.
 */
class Post_Error_Handler {
	 ...
	 */
	public static function get_synced_posts_of_blog_with_errors( $blog_id = 0, $repair_posts = false, $query_args = null ) {
	 ...
	 * @return Synced_Post[]    With @param object error
	 */
	public static function get_network_synced_posts_with_errors( $repair_posts = false, $query_args = null ) {
		$error_posts = array();
		foreach ( Multisite_Manager::get_all_blogs() as $blog_id => $blog_args ) {
			$error_posts = array_merge(
				self::get_synced_posts_of_blog_with_errors( $blog_id, $repair_posts, $query_args ),
				$error_posts
			);
		}
		return $error_posts;
