 ...
 *
 * This file enables advanced post imports inside WordPress.
 ...
 *
 * The import contains a JSON file that holds all post data as
 ...
 *
 * @action 'contentsync_ajax_mode_check_post_import'
 *
 * @param array $data   holds the $_FILES['data']
 */
public function check_import( $data ) {

	Logger::add( '========= CHECK IMPORT POST DATA =========', $data );
	...
	if ( isset( $data['error'] ) && $data['error'] > 0 ) {
		...
		\Contentsync\admin_ajax_return_error( $file_errors[ $data['error'] ] );
	}
	...
	if ( $filetype != 'application/zip' && $filetype != 'application/x-zip-compressed' ) {
		\Contentsync\admin_ajax_return_error( __( 'Please select a valid ZIP archive.', 'contentsync_hub' ) );
	}
	...
	if ( ! is_array( $post_data ) ) {
		\Contentsync\admin_ajax_return_error( $post_data );
	}
	...
	admin_ajax_return_success( $return );
}

/**
 * Handle the ajax import action.
 *
 * @action 'contentsync_ajax_mode_post_import'
 *
 * @param array $data   holds the $_POST['data']
 */
public function handle_import( $data ) {

	Logger::add( '========= HANDLE IMPORT =========', $data );
	...
	if ( empty( $filename ) ) {
		\Contentsync\admin_ajax_return_error( __( 'The file name is empty.', 'contentsync_hub' ) );
	}
	...
	admin_ajax_return_success( sprintf( __( "Post file '%s' has been imported successfully.", 'contentsync_hub' ), $filename ) );
}
...
public static function import_posts( $posts, $conflict_actions = array(), $zip_file = '' ) {
	...
	foreach ( $posts as $post_id => $post ) {
		...
		$translation_analysis = Translation_Manager::analyze_translation_import( $post, self::$posts );
		...
		if ( $conflict_action === 'replace' ) {
			...
			if ( $existing_post_id ) {
				...
				if ( $skip ) {
					...
					self::$posts[ $post_id ] = $existing_post_id;
	...
	foreach ( $posts as $old_post_id => $post ) {
		 ...
		 */
		self::set_post_hierarchy( $new_post_id, $post );
		...
		if ( $thumbnail_id  = get_post_thumbnail_id( $new_post_id ) ) {
			...
			if ( $result ) {
				Logger::add( sprintf( "  - thumbnail ID changed from '%s' to '%s'", $thumbnail_id, self::$posts[ $thumbnail_id ] ) );
			} else {
...
public static function create_post( $postarr, $post, $zip_file = '' ) {
	...
	// attachment
		...
		$new_post_id = self::insert_attachment( $postarr, $media, $zip_file );
...
public static function insert_attachment( $postarr, $media_file_info, $zip_file = '' ) {
	...
	if ( ! empty( $zip_file ) ) {
		...
		$file_data = Helper::get_file_contents( $media_file_info['url'] );
	}
	...
	if ( ! $file ) {
		new WP_Error( 'file', 'Attachment file path could not be created.' );
	}
 ...
 */
public static function add_attachment_files_to_replace_strings( $media_file_info, $new_file ) {
	...
	self::$collected_replace_strings[ $old_file_path ] = $new_file_path;
	...
	foreach ( $meta_value as $key => $value ) {
		if ( preg_match( '/\{\{(.+?)\}\}/', $value, $matches ) ) {
			...
			if ( is_numeric( $inner ) ) {
				...
				Logger::add( sprintf( "  - ID '%s' in the field '%s' was replaced with '%s'", $inner, $key, $replace_string ) );
			} else {
				$meta_value[ $key ] = self::replace_strings( $value, $post_id, false );
				Logger::add( sprintf( "  - All strings in the field '%s' were replaced", $key ) );
			}
		}
	}
...
public static function set_translations( $post_id, $post ) {
	...
	// - Mapping original IDs to new IDs via self::$posts
	// - WPML trid lookups
