...
function check_synced_post_import( $gid ) {
	...
	$conflicts = get_conflicting_post_options( $posts );
 ...
 */
function get_conflicting_post_options( $posts ) {

	// get conflicting posts
	$conflicts = get_conflicting_posts( $posts );

	if ( count( $conflicts ) > 0 ) {
		foreach ( $conflicts as $post_id => $post ) {

			// get the post link to display in the backend
			$conflicts[ $post_id ]->post_link = get_post_link_html( $post );
			/**
 ...
 */
function get_conflicting_posts( $posts ) {
 ...
 */
function get_post_link_html( $post ) {
 ...
 * @return array
 */
function get_conflicting_post_selections( $conflicts ) {
