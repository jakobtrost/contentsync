...
class Queue_List_Table extends \WP_List_Table {
	...
	public function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'import_action':
				...
				echo Admin_Render::make_admin_icon_status_box( $color, $text, false );
				break;
	...
	public function process_row_action() {
		...
		if ( isset( $_GET['run_now'] ) && check_admin_referer( 'contentsync_run_now' ) ) {
			...
			if ( $result !== false ) {
				Admin_Render::render_admin_notice( __( 'Distribution started successfully.', 'contentsync' ), 'success' );
			} else {
				Admin_Render::render_admin_notice( __( 'Failed to start distribution.', 'contentsync' ), 'error' );
			}
		}
		...
		if ( isset( $_GET['reschedule'] ) && check_admin_referer( 'contentsync_reschedule' ) ) {
			...
			if ( ! is_wp_error( $result ) ) {
				Admin_Render::render_admin_notice( __( 'Distribution rescheduled successfully.', 'contentsync' ), 'success' );
			} else {
				Admin_Render::render_admin_notice( sprintf( __( 'Failed to reschedule distribution: %s', 'contentsync' ), $result->get_error_message() ), 'error' );
			}
		}
		...
		if ( isset( $_GET['delete_queue'] ) && check_admin_referer( 'contentsync_delete_queue' ) ) {
			...
			if ( $item && $item->delete() ) {
				Admin_Render::render_admin_notice( __( 'Distribution deleted successfully.', 'contentsync' ), 'success' );
			} else {
				Admin_Render::render_admin_notice( __( 'Failed to delete distribution.', 'contentsync' ), 'error' );
			}
		}
		...
		if ( isset( $_GET['delete_all_queues'] ) && check_admin_referer( 'contentsync_delete_all_queues' ) ) {
			...
			Admin_Render::render_admin_notice( $message, $message_type );
