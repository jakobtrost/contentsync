...
contentSync.blockEditorTools = new (function () {
	...
	this.setNotice = (notice) => {
		if (notice && notice.length > 0) {
			dispatch('core/notices').removeNotices(
				select('core/notices')
					.getNotices()
					.map((n) => n.id)
			);
			...
			dispatch('core/notices').createNotice(notice[0], notice[1], notice[2]);
		}
		// remove all notices
		else {
			dispatch('core/notices').removeNotices(
	...
	this.saveOptions = function (post_id, options, canonical_url) {
		apiFetch({
			...
			.catch((err) => {
				...
				contentSync.blockEditorTools.showSnackbar('Failed to save options', 'error', true);
			});
	};
	...
	/**
	 * Render a status box
	 * @param {string} status Can be one of: export, import, error.
	 * @param {string} text Text of the status box
	 * @returns {React.ReactNode} The status box
	 */
