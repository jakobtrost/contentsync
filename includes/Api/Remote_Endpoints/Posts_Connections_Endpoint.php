...
class Posts_Connections_Endpoint extends Remote_Endpoint_Base {
	...
	public function add_post_connection_to_connection_map( $request ) {
		...
		list( $_blog_id, $_post_id, $net_url ) = Synced_Post_Utils::explode_gid( $request['gid'] );
		if ( $_post_id !== null ) {
			...
			$result = Post_Connection_Map::add_or_remove( $gid, $args, true, $site_url );
			if ( $result ) {
	...
	public function remove_post_connection_from_connection_map( $request ) {
		...
		if ( $_post_id !== null ) {
			...
			$result = Post_Connection_Map::add_or_remove( $gid, $args, false, $site_url );
			if ( $result ) {
