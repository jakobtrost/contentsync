...
class Global_List_Table extends \WP_List_Table {
		...
		if ( is_multisite() && is_network_admin() ) {
			...
			foreach ( $items as $post ) {
				if ( isset( $post->language ) && ! empty( $post->language ) ) {
					...
					$tool = Main_Helper::get_translation_tool();
					Main_Helper::restore_blog();
					// compare to this
					if ( Main_Helper::get_translation_tool() != $tool ) {
						switch ( $tool ) {
							case 'wpml':
	...
	public function single_row( $post ) {
		...
		$item->local_post     = is_network_admin() ? false : Main_Helper::get_local_post_by_gid( $gid );
		...
		if ( empty( $item->site_url ) ) {
			if ( empty( $root_net_url ) ) {
				...
				if ( isset( $this->posts[ $root_net_url ] ) ) {
					$_post = array_filter(
		...
		} else if ( $item->post_type == "wp_navigation") {
			$item->post_links = array(
		...
		} else if ( $item->post_type == "wp_global_styles" ) {
			$item->post_links = array(
	...
	public function column_default( $item, $column_name ) {
		...
		list( $root_blog_id, $_root_post_id, $root_net_url ) = Main_Helper::explode_gid( $gid );
	...
	public function process_bulk_action() {
		...
		foreach ( $gids as $gid ) {
			...
			elseif ( $bulk_action === 'import' ) {
				...
				// // already imported
				// if ( $post = Main_Helper::get_local_post_by_gid( $gid ) ) {
				// 	continue;
				// }
	...
	public function sortby_site_url_asc( $a, $b ) {
		...
		// get a
		if ( isset( $a->meta ) ) {
			list( $a_blog_id, $a_post_id, $a_net_url ) = Main_Helper::explode_gid( $a->meta['synced_post_id'] );
		} else {
			$a_blog_id = $a->blog_id;
		...
		// get b
		if ( isset( $b->meta ) ) {
			list( $b_blog_id, $b_post_id, $b_net_url ) = Main_Helper::explode_gid( $b->meta['synced_post_id'] );
		} else {
			$b_blog_id = $b->blog_id;
