...
class Admin {
	...
	public function save_overview_screen_options($status, $option, $value) {
		...
		if ( 'globals_per_page' == $option ) return $value;
	...
	public static function get_global_notice_content( $post_id, $mode='classic' ) {
		...
		if ( !Main_Helper::is_error_repaired( self::$error ) ) {
			...
			if ( $status === 'linked' ) {
				 ...
				 */
				$review                = get_synced_post_review_by_post( $post_id, get_current_blog_id() );
				...
				else if ( $review_status === 'in_review' ) {
					$status_pill_class = ' status_pill_neutral';
					$status_pill_title = __( 'In Review', 'contentsync' );
				}
				else if ( $review_status === 'denied' ) {
				...
				else if ( ! empty( self::get_clusters_including_this_post( $post_id ) ) ) {
					$post_count_text = sprintf(
						_n(
							"1 active cluster",
							"%s active clusters",
			...
			// site editor notice arguments
				...
				return array(
					...
					array(
						...
						'__unstableHTML' => true,
						'icon'           => $icon,
						'actions'        => array_map( function($button) {
							return array(
	...
	public function add_meta_box() {
		...
		if ( get_current_screen()->action === 'add' ) return false;
		
		$posttypes = Main_Helper::call_post_export_func( 'get_supported_post_types' );
	...
	public static function get_global_metabox_content( $post_id ) {
		...
		if ( empty($status) || empty($gid) ) {
			...
			$return .= "<div id='contentsync_similar_posts'>
				<div class='found hidden'>
					...
					<ul class='contentsync_box_list' data-item='".preg_replace("/\s{2,}/", "", esc_attr(
						"<li>
							<span class='flex'>
			...
			self::$overlay_warnings['contentsync_export'] = "<div class='export_warning_similar_posts hidden' style='margin:1em 0 -2em;'>".Main_Helper::render_info_box( array(
		...
		// global post
			 ...
			 */
				...
				/**
				 * @since 1.7 'contentsync_options' can now be edited.
				 */
				if ( Main_Helper::current_user_can_edit_global_posts( $status ) ) {
					$options            = Main_Helper::get_contentsync_meta( $post_id, 'contentsync_options' );
					$editable_options   = self::get_contentsync_options_for_post( $post_id );
					// debug( $options );
					$return .= "<input type='checkbox' class='hidden _contentsync_options_toggle' id='_contentsync_options_toggle1'/>
					<label class='contentsync_options_toggle' for='_contentsync_options_toggle1'><span class='dashicons dashicons-admin-generic'></span></label>
					<div class='editable_contentsync_options contentsync-gray-box'>
						...
						foreach( $editable_options as $option ) {
							...
							$return .= "<input type='hidden' name='editable_contentsync_options[{$option['name']}]' value='off' />
								<label><input type='checkbox' name='editable_contentsync_options[{$option['name']}]' {$checked} />{$option['title']}</label><br>";
						}
						if ( is_array($connection_map) && count($connection_map) > 0 ) {
							$return .= Main_Helper::render_info_box( array(
				...
				if ( !empty( self::get_clusters_including_this_post( $post_id ) ) ) {
					...
					if ( $cluster_has_reviews ) {
						...
						$return .= "<input type='checkbox' class='hidden _contentsync_options_toggle' id='_contentsync_options_toggle2'/>";
						$return .= "<label class='contentsync_options_toggle button' for='_contentsync_options_toggle2'>".__("Toggle review history", "contentsync")."</label>";
						$return .= "<ul class='editable_contentsync_options contentsync_box_list'>";
						$reviews = get_contentsync_all_post_reviews_by_post( $post_id, get_current_blog_id() );
						// debug($reviews);
						foreach ( $reviews as $review ) {
							...
							if ( !empty($messages) ) {
								...
								foreach ( $messages as $message ) {
									...
									$inner = $message->get_date() . "<br><strong>" . sprintf( __( "%s by %s", "contentsync" ), $message->action, $reviewer ) . "</strong>";
							...
							if ( empty($messages) || $latest_message->action != $review->state ) {
								...
								if ( $review->state != 'new' && $review->state != 'in_review' ) {
									$inner = sprintf( __( "%s by %s", "contentsync" ), $review->state, 'N/A' )."<br>";
								}
								$details .= "<li>".$inner."</li>";
							}
	...
	public function edit_body_class( $classes ) {
		...
		else if ( ! empty( $status ) && ! Main_Helper::current_user_can_edit_global_posts( $status ) ) {
			$classes .= ' contentsync-locked';
		}
	...
	public function setup_columns() {
		$post_types = (array) Main_Helper::call_post_export_func( 'get_supported_post_types' );
	...
	public function render_column( $column_name, $post_id ) {
		if ( 'contentsync_status' === $column_name ) {
			...
			$supported_post_types = Main_Helper::call_post_export_func( 'get_supported_post_types' );
			...
			else if ( Main_Helper::current_user_can_edit_global_posts( 'root' ) ) {
	...
	public function handle_bulk_action_make_global( $redirect_to, $doaction, $post_ids ) {
		...
		foreach ( $post_ids as $post_id ) {
			$gid = \Contentsync\Contents\Actions::make_post_global( $post_id, $export_args );
	...
	public function add_overlay_contents( $contents ) {
		...
		$export_options = self::get_contentsync_options_for_post( $post_id );
		...
		$overlay_contents = array(
			...
			'contentsync_delete' => array(
				'confirm' => array(
					...
					'content'   => Main_Helper::render_info_box( array(
	 ...
	 * @return array
	 */
	public static function get_contentsync_options_for_post( $post_id_or_object=null ) {
		...
		if ( $post ) {
			...
			if ( $post->post_type === 'attachment' ) {
				unset( $contentsync_options['nested'], $contentsync_options['menus'] );
			}
