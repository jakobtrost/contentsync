...
class Translation_Tool_WPML extends Translation_Tool_Base {
	...
	public function set_post_language( $post_id, $language_code ) {
		...
		if ( function_exists( 'contentsync_wpml_set_element_language_details' ) ) {
			$result = contentsync_wpml_set_element_language_details( array(
				'element_id'    => $post_id,
		...
		$this->log( '  - standalone WPML functions not available.' );
		return false;
	}
	...
	public function set_translations_from_import( $post_id, $language_code, $original_post_ids, $language_args, $imported_post_map ) {
		...
		if ( ! $post ) {
			$this->log( '  - failed to set translations: post not found.' );
			return false;
		}
		...
		if ( $this->is_plugin_loaded() ) {
			...
			$this->log( sprintf( "  - set translation for post '%s' with trid '%s'.", $post_id, $trid ) );
			return true;
		}
		...
		if ( function_exists( 'contentsync_wpml_set_element_language_details' ) ) {
			...
			if ( $result ) {
				...
				$this->log( sprintf( "  - failed to set translation for post '%s'.", $post_id ) );
				return false;
			}
		}
	...
	public function analyze_translation_import( $post, $already_imported_posts = array() ) {
		...
		if ( count( $supported_translations ) > 0 ) {
			$this->log( "  - There is at least 1 supported language for this post: " . implode( ', ', $supported_translations ) );
	...
	public function switch_to_language_context( $post ) {
		...
		if ( ! $sitepress || ! is_object( $sitepress ) || ! method_exists( $sitepress, 'switch_lang' ) ) {
			$this->log( sprintf( "  - the global \$sitepress object is not initialized, so we could not switch to the language '%s'.", $post_lang ) );
			return false;
		}

		// Check whether the language is supported on the current site
		if ( ! $this->is_language_supported( $post_lang ) ) {
			$this->log( "  - the language '$post_lang' is not part of the supported languages (" . implode( ', ', $this->get_language_codes() ) . ')' );
			return false;
		}
		...
		$this->log( sprintf( "  - switched the language to '%s'.", $post_lang ) );
		return true;
	}
}
