...
class Post_Export_Admin {
	...
	public function add_import_page_title_action() {
		...
		wp_register_script(
			'contentsync-post-export-script',
			plugin_dir_url( __FILE__ ) . 'assets/js/post-export.js',
			array( 'jquery', 'contentsync-admin-script' ),
			CONTENTSYNC_VERSION
		);
		wp_enqueue_script( 'contentsync-post-export-script' );

		wp_add_inline_script(
			'contentsync-post-export-script',
			'jQuery(function() {
				contentsync.tools.addPageTitleAction( "⬇&nbsp;' . __( 'Import', 'contentsync_hub' ) . '", { onclick: "contentsync.postExport.openImport();" } );
			});',
			'after'
		);
	}
	...
	public function maybe_enable_debug_mode() {
		...
		if ( isset( $_GET['post'] ) ) {
			...
			$posts = \Content_Sync\Main_Helper::prepare_global_post_for_import( strval( $_GET['contentsync_export_debug'] ) );
			debug( $posts );
		}
