...
class Site_Editor {
	...
	public function rest_api_init() {
		...
		register_rest_route( 'contentsync/v1', '/get_post_info', array(
			'methods'             => 'POST',
			'callback'            => array( $this, 'get_post_info' ),
			'permission_callback' => '__return_true',
		) );

		register_rest_route( 'contentsync/v1', '/save_options', array(
			'methods'             => 'POST',
	...
	public function get_post_info( $request ) {
		...
		if ( $params = $request->get_params() ) {
			...
			$contentsync_options = get_post_meta( $post_id, 'contentsync_options', true );
			...
			$available_options = Admin::get_contentsync_options_for_post( $post_id );
			...
			$contentsync_options = array_merge( $default_options, $contentsync_options ?: array() );
			...
			$data = array(
				'post' => array_merge(
					...
					( $status === 'root' ? array(
						...
						'options'       => $contentsync_options ?: array(),
						'canonicalUrl'  => $canonical_url,
						'availableOptions' => $available_options,
						'cluster'       => array_map( function( $cluster ) use ( $connection_map ) {
							if ( isset( $cluster->destination_ids ) && is_array( $cluster->destination_ids ) ) {
								$cluster->destination_ids = array_map( function( $blog_id ) use ( $connection_map ) {
		...
		return json_encode( [ "status" => 400, "message" => "Could get post infos" ] );
	}
	...
	public function save_options( $request ) {
		...
		if ( $params = $request->get_params() ) {
			...
			if ( $post_id <= 0 ) {
				return json_encode( [ "status" => 400, "message" => "Invalid post ID" ] );
			}

			// Check if user can edit this post
			if ( ! Main_Helper::current_user_can_edit_global_posts( 'root' ) ) {
				return json_encode( [ "status" => 403, "message" => "Permission denied" ] );
			}
		...
		return json_encode( [ "status" => 400, "message" => "Invalid parameters" ] );
	}
	...
	public static function get_numeric_post_id( $site_editor_post_id ) {
		...
		if ( count( $parts ) === 2 ) {
			...
			$posts = Main_Helper::get_posts( array(
				'name'          => $parts[1],
