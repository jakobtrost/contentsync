...
class Cluster_Admin {
	...
	public function clusters_add_screen_options() {
		...
		if ( !class_exists( 'Cluster_List_Table' ) ) {
			require_once __DIR__.'/cluster-list-table.php';
		}
	...
	public function cluster_save_screen_options( $status, $option, $value ) {
		...
		if ( 'clusters_per_page' == $option ) return $value;
	...
	public function render_cluster_edit_page() {
		...
		// $cluster = get_contentsync_cluster($cluster_id);
					...
					<div>
						<div>
							<div>
								...
								<?php echo Main_Helper::render_info_box( array(
	...
	public function save_cluster_settings() {
		...
		if ( isset( $_POST['save'] ) && $nonce && wp_verify_nonce( $nonce, $nonce_action ) && $cluster_id ) {
			...
			$cluster_posts_before   = get_contentsync_cluster_posts_per_blog( $cluster_id );
			$cluster_before         = get_contentsync_cluster_by_id( $cluster_id );
			...
			if ( isset( $_POST['conditions'] ) ) {
				...
				$existing_conditions = get_contentsync_content_conditions_by_cluster_id( $cluster_id );
				...
				foreach ( $content_conditions as $index => $condition ) {
					...
					if ( !isset( $condition['ID'] ) || $condition['ID'] == 'hidden' ) continue;
					...
					if ( $condition_id == 'new' ) {
						...
						$condition_id               = insert_contentsync_content_condition( $condition );
					}
					else {
						$condition['ID']  = $condition_id;
						$condition_before = get_contentsync_content_condition_by_id( $condition_id );
						// if (
						...
						// 	$conditon_posts = get_posts_by_contentsync_content_condition( $condition_before, true );
						...
						// 	}
						// }
						$res = update_contentsync_content_condition( $condition );
					}
				...
				if ( $existing_conditions && is_array( $existing_conditions ) ) {
					...
					foreach ( $existing_conditions as $existing_condition ) {
						...
						if ( !in_array( $existing_condition['ID'], $content_condition_ids ) ) {
							delete_contentsync_content_condition( $existing_condition['ID'] );
						}
					}
				}
			...
			$result = update_contentsync_cluster( $cluster );
			...
			if ( $result ) {
				...
				if (
					...
					// distribute posts
					$distribute_result = Distributor::distribute_contentsync_cluster_posts( new \Cluster( (object) $cluster ), array(
					...
					if ( $has_date_condition ) {
						schedule_contentsync_cluster_date_check();
					}
				}
			}
		}
		...
		if ( $result ) {
			Main_Helper::show_message( sprintf(
	...
	public static function log( $log, $action = '' ) {
		if ( $action == 'start' ) ob_start();
		echo str_pad( $log, 4096 );
		ob_flush();
		flush();
		if ( $action == 'end' ) ob_end_flush();
	}
	...
	public function render_cluster_settings_box( $cluster ) {
		...
		$all_blogs = \Greyd\Hub\Hub_Helper::get_all_networks();
		...
		foreach ( $remote as $network_url => $network ) {
			...
			foreach ( $network as $blog ) {
				// debug( $blog );
				if ( !isset( $blog['blog_id'] ) || $blog['blog_id'] == 0 ) continue;
				$title = urlencode( $blog['name'] ).' – '.$blog['domain'];
	...
	public function render_cluster_screen_conditions_box( $cluster ) {
		...
		$conditions = $cluster['content_conditions']; // get_contentsync_content_conditions_by_cluster_id( $cluster['ID'] );
	...
	public function render_condition_row( $condition, $index ) {
		...
		// get blogs for the select
		$blog_options = '';
		$blogs        = \Greyd\Hub\Hub_Helper::get_basic_blogs();
		...
		switch ( $mode ) {
			case 'retain_post_connections':
				...
				foreach ( $cluster->content_conditions as $condition ) {
					delete_contentsync_content_condition( $condition->ID );
				}

				$result = delete_contentsync_cluster( $cluster_id );
	...
	public function create_cluster() {
		...
		// $result = false;
		$cluster_id = insert_contentsync_cluster(
			array(
				'title' => $data['title'],
			)
		);
	...
	public function post_reviews_add_screen_options() {
		...
		if ( !class_exists( 'Post_Review_List_Table' ) ) {
			require_once __DIR__.'/post-review-list-table.php';
		}
	...
	public function post_reviews_save_screen_options( $status, $option, $value ) {
		...
		if ( 'post_reviews_per_page' == $option ) return $value;
	...
	public function get_stage_posttype_data() {
		...
		foreach ( $blogs as $blog ) {
			 ...
			 * @since 2.18.0
			 */
			$translation_tool = Main_Helper::get_translation_tool();
