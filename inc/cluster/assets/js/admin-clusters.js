...
contentsync.clusters = new function() {
	...
	this.wizard = new function() {
		...
		this.init = () => {
			...
			contentsync.tools.addPageTitleAction( __('Add Cluster', 'contentsync'), {onclick: 'contentsync.clusters.wizard.openWizard()'});
			...
			wizard.querySelector('.button.create').addEventListener('click', (e) => {
				const clusterID = this.createCluster( wizard.querySelector('input[name="create_cluster"]').value )
				.then( (clusterID) => {
					if (clusterID > 0) {
						const url = location.href.split("wp-admin");
						const editURL = url[0]+contentsync_admin.admin_url_base+"?page=contentsync_clusters&cluster_id="+clusterID;
	...
	this.deletionOverlay = new function() {
		...
		this.init = () => {
			...
			deleteButtons.forEach((button) => {
				button.addEventListener('click', (e) => {
					...
					contentsync.tools.confirm("contentsync_delete_cluster", title.textContent, this.deleteCluster, [button]);
				});
			});
		}
		...
		this.getData = (button) => {
			...
			const selectedRadio = form.querySelector('input[name="contentsync_cluster_deletion_mode"]:checked');
		...
		this.deleteCluster = ( elem ) => {
			...
			.then( ( data ) => {
				if ( data ) {
					if ( this.debug ) console.log( data );
					if ( data.success ) {
						contentsync.tools.triggerOverlay( true, { "type": "reload", "css": "contentsync_delete_cluster" } );
					} else {
						contentsync.tools.triggerOverlay( true, { "type": "fail", "css": "contentsync_delete_cluster", "replace": data } );
					}
				}
			});
		}
	...
	this.multiselect = new function () {
		 ...
		 * @since 1.4.5 @see popups_admin.js
		...
		this.setAutotags = function (el) {
			...
			for (let val of value) {
				if (typeof tagsData[val] !== 'undefined') {
					...
					tags += '<span class="tag" data-value="' + val + '">' + title + '<span class="tag_close dashicons dashicons-no-alt"></span></span>';
				}
			}
		...
		this.initEvents = function (wrapper) {
			...
			$(wrapper).find( '.tag_suggest.option' ).on( 'click', function ( e ) {
				...
				var is_cat = ( typeof id === 'string' && id.indexOf( 'cat_' ) ) === 0 ? theme_wordings.vc.tags.category + ": " : "";
	...
	this.conditions = new function () {
		this.init = () => {
			...
			const enableReviewsAuthorSelect = document.getElementById('enable_reviews_author_select');
			...
			const dateModeSelect = document.querySelectorAll('.filter_section[data-mode="date"] .date_mode');
		...
		this.updateRowNumbers = () => {
			...
			rows.forEach( (row) => {
				...
				inputs.forEach( (input) => {
					...
					input.setAttribute('name', name.replace(/(conditions\[)(.*?)(\]\[)/g, 'conditions['+i+']['));
				}
				);
		...
		this.updateHeight = (sub, rowHeight) => {
			rowHeight = (typeof rowHeight === "undefined") ? sub.querySelector('table').getBoundingClientRect().height : rowHeight;
		...
		this.handlePosttypeChange = (select) => {
			...
			this.updateTaxonomyOptions(data[blogID].post_types[posttype].taxonomies, sub.querySelector('.taxonomy_select'))
			this.handleTaxonomyChange(sub.querySelector('select.taxonomy_select'));
		}
		...
		this.handleTaxonomyChange = (select) => {
			...
			let termsData = {};
			termsData.Terms = {};
			if ( typeof data[blogID].post_types[selectedPosttype].taxonomies !== 'undefined'
				&& typeof data[blogID].post_types[selectedPosttype].taxonomies[taxonomy] !== 'undefined') {
				termsData.Terms = data[blogID].post_types[selectedPosttype].taxonomies[taxonomy].terms;
			}
		...
		this.handleDateModeChange = (fieldset) => {
			...
			const inputSections = filterSection.querySelectorAll('.input_section[data-date-mode="'+selectedRadioButton.value+'"]');
			...
			if ( posttypeData && posttypeData.is_dynamic && !posttypeData.is_global ) {
				...
				for ( var i=0; i<conditions.length; i++) {
					...
					if ( conditions[i].value == blogIndex.value ) {
						...
						if ( wrapper?.querySelector('.posttype_select_wrapper select')?.value == 'tp_posttypes' ) {
							msg = messages.not_global+'<br>'+messages.not_all;
							if( wrapper.querySelector('.make_posts_global_automatically')?.checked ) {
								msg = "";
							}
						}
					}
				...
				if ( msg != "" ) {
					// show message
					msg = msg.split( '__pt__' ).join( posttypeData.title ).split( '__blog__' ).join( blogIndex.textContent );
					// console.warn(msg);
