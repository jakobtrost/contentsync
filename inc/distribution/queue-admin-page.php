 ...
 * Queue admin page.
 *
 * This file defines the `Admin` class used to render and manage the
 * administrative interface for the distribution queue. It registers a
 * submenu under the Content Sync menu, initialises the queue list table
 * used to display pending distribution items, and provides actions and
...
class Admin {
	...
	public function check_stuck_items() {
		...
		if ( strpos( $screen->id, 'global-content' ) === false && strpos( $screen->id, 'global_content' ) === false ) {
			return;
		}
