...
greyd.distribution = new function() {
	...
	this.queue = new function() {
		...
		this.togglePause = () => {
			...
			const pauseButton = document.querySelector('#contentsync-pause-stuck');
			if (pauseButton) {
				pauseButton.textContent = this.isPaused ? distributionData.i18n.resume : distributionData.i18n.pause;
			}
		...
		this.updateProgress = () => {
			...
			if (progressCount) {
				progressCount.textContent = distributionData.i18n.progressLabel.replace('%s', processed).replace('%s', total);
			}
		}
			...
			if (summaryText) {
				...
				if (this.currentIndex < this.stuckItems.items.length) {
					...
					if (this.errorCount === 0) {
						summaryText.textContent = distributionData.i18n.summaryAllSuccess.replace('%s', total).replace('%s', this.stuckItems.items.length);
					} else {
						summaryText.textContent = distributionData.i18n.summarySomeFailed.replace('%s', this.successCount).replace('%s', total).replace('%s', this.errorCount).replace('%s', remaining);
					}
				} else {
		...
		this.processNextItem = async () => {
			...
			try {
				...
				if (result.success) {
					...
					this.errorCount++;
					this.addLogEntry(`Item ${item.id}: ${result.data.message || distributionData.i18n.failedToProcess}`, 'failed');
					this.currentIndex++;
					this.updateProgress();
					this.processNextItem();
				}
				...
				this.errorCount++;
				this.addLogEntry(`Item ${item.id}: ${distributionData.i18n.errorProcessingItem}`, 'failed');
				this.currentIndex++;
				this.updateProgress();
				this.processNextItem();
...
function updateTableRowStatus(itemId, status) {
	...
	const checkbox = document.querySelector('input[type="checkbox"][name="post[]"][value="' + itemId + '"]');
	if (!checkbox) return;
